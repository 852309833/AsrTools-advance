...
class JianYingASR(BaseASR):
    ...
    def _run(self, callback=None):
        ...
        for _ in range(300):
            ...
            raise TimeoutError("剪映识别超时（约5分钟），请稍后重试或换用必剪接口")
    ...
    def _generate_sign_parameters(self, url: str, pf: str = '4', appvr: str = '4.0.0', tdid='') -> \
        ...
        for attempt in range(5):
            try:
                response = requests.post(get_sign_url, json=data, timeout=30)
        ...
        raise RuntimeError(f"剪映签名服务不可用，请检查网络后重试: {last_error}") from last_error
